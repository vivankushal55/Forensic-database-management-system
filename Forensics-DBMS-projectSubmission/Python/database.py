...
mydb = mysql.connector.connect(host="localhost", user="root", password="Rossi@4646", database="Forensics")
c = mydb.cursor()
...
def get_criminalcase_no(): 
    c.execute('SELECT CrimeID FROM criminalcase') 
    data = c.fetchall()
    return data
def delRec(id, choice):
    if choice=="Automobile":
        ...
        command = 'DELETE FROM criminalcase where CrimeID = "'+id+'"'
    print(command)
    c.execute(command)
    mydb.commit()
...
def updateStatus1(id, UpdateChoice, choice,Name):
    if choice=="Criminal":
        command = 'UPDATE CRIMINAL set ' + UpdateChoice+' = "'+Name +'" where CID = "' + id + '"'
    elif choice=="Cases":
        command = 'UPDATE CASES set '+ UpdateChoice+' = "' +Name+ '" where CaseID = "' + id + '"'
    elif choice=="Automobile":
        command = 'UPDATE Automobile set '+ UpdateChoice+' = "' +Name+ '" where AID = "' + id + '"'
    elif choice=="Ballistics":
        command = 'UPDATE Ballistics set '+ UpdateChoice+' = "' +Name+ '" where B_ID = "' + id + '"'
    elif choice=="Drugs":
        command = 'UPDATE Drugs set '+ UpdateChoice+' = "' +Name+ '" where NDC_NO = "' + id + '"'
    elif choice=="Paint":
        command = 'UPDATE Paint set '+ UpdateChoice+' = "' +Name+ '" where PID = "' + id + '"'
    elif choice=="CriminalCase":
        command = 'UPDATE CriminalCase set '+ UpdateChoice+' = "' +Name+ '" where CrimeID = "' + id + '"'
